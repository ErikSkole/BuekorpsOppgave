...
async function fetchUsers() {
    try {
        ...
        ['Rolle', 'Bruker', 'Fornavn', 'Etternavn', 'Tlf', 'Email', 'Rediger', 'Slett'].forEach(text => {
        ...
        users.forEach(user => { // For each user in the database, create a table row with the username, role and an edit button
            ...
            [user.role, user.username, user.first_name, user.last_name, user.phone, user.email].forEach(text => {
...
function closeEditModal(value) { // Function to close the edit modal form
    document.querySelector('.editModal').classList.toggle('hidden');
}

function closeCreateModal(value) { // Function to close the edit modal form
